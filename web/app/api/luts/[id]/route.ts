...
export async function DELETE(
    ...
    try {
        ...
        await db.collection("luts").deleteOne({ _id: new ObjectId(id) });
...
export async function PATCH(
    ...
    try {
        ...
        const db = await getDb();
        await db.collection("luts").updateOne(
            { _id: new ObjectId(id) },
            { $set: { name, updatedAt: new Date() } },
        );
        const updated = await db.collection("luts").findOne({ _id: new ObjectId(id) });
