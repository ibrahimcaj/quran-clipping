...
function makeAssCard(
    ...
    return [
        ...
        `PlayResX: ${TEXT_CARD_SIZE}`, `PlayResY: ${TEXT_CARD_SIZE}`,
        "ScaledBorderAndShadow: yes", "WrapStyle: 0", "",
        "[V4+ Styles]",
        "Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding",
        `Style: Default,Geeza Pro,${titleFontSize},&H00FFFFFF,&H00FFFFFF,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,0,0,5,0,0,0,1`,
        "", "[Events]",
...
function run(cmd: string, args: string[]): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        proc.on("close", (code) => code === 0 ? resolve() : reject(new Error(err.trim() || `${cmd} exited ${code}`)));
    });
}
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const safeTitleFontSize = Math.max(8, Math.min(200, Math.round(titleFontSize)));
        const safeSubtitleFontSize = Math.max(8, Math.min(200, Math.round(subtitleFontSize)));
        ...
        const video = await db.collection("videos").findOne({ _id: new ObjectId(videoId) }) as { filePath?: string } | null;
        ...
        if (lutId && ObjectId.isValid(lutId)) {
            const lut = await db.collection("luts").findOne({ _id: new ObjectId(lutId) }) as { filePath?: string } | null;
            if (lut?.filePath && fs.existsSync(lut.filePath)) lutPath = lut.filePath;
        }

        let overlayPath: string | null = null;
        if (overlayId && ObjectId.isValid(overlayId)) {
            const overlay = await db.collection("overlays").findOne({ _id: new ObjectId(overlayId) }) as { filePath?: string } | null;
            if (overlay?.filePath && fs.existsSync(overlay.filePath)) overlayPath = overlay.filePath;
        }

        const cfg = await db.collection("configuration").findOne({ type: "video" }) as Record<string, unknown> | null;
        const resolvedVignette = typeof vignette === "number"
            ? Math.max(0, Math.min(1, vignette))
            : (typeof cfg?.vignette === "number" ? cfg.vignette : 0);
        const resolvedExposure = typeof exposure === "number"
            ? Math.max(-3, Math.min(3, exposure))
            : (typeof cfg?.exposure === "number" ? cfg.exposure : 0);
        const resolvedSaturation = typeof saturation === "number"
            ? Math.max(0, Math.min(3, saturation))
            : (typeof cfg?.saturation === "number" ? cfg.saturation : 1);
        const resolvedOverlayId = overlayId === undefined
            ? (cfg?.overlayId?.toString?.() ?? cfg?.overlayId ?? null)
            : overlayId;
        const resolvedOverlayBlendMode = typeof overlayBlendMode === "string"
            ? overlayBlendMode
            : (typeof cfg?.overlayBlendMode === "string" ? cfg.overlayBlendMode : "normal");

        if (!overlayPath && resolvedOverlayId && ObjectId.isValid(String(resolvedOverlayId))) {
            const overlay = await db.collection("overlays").findOne({ _id: new ObjectId(String(resolvedOverlayId)) }) as { filePath?: string } | null;
            if (overlay?.filePath && fs.existsSync(overlay.filePath)) overlayPath = overlay.filePath;
        }
        ...
        if (resolvedVignette > 0) frameFilters.push(`vignette=a=${(Math.PI * 0.9 * resolvedVignette).toFixed(4)}`);
        if (resolvedExposure !== 0) frameFilters.push(`exposure=exposure=${resolvedExposure.toFixed(2)}`);
        if (resolvedSaturation !== 1) frameFilters.push(`eq=saturation=${resolvedSaturation.toFixed(2)}`);

        const framePng = path.join(workDir, "frame.png");
        await run("ffmpeg", ["-y", "-ss", offset.toFixed(3), "-i", video.filePath, "-frames:v", "1", "-vf", frameFilters.join(","), framePng]);
        ...
        await run("ffmpeg", [
            "-y",
            "-f", "lavfi", "-i", `color=c=black@0.0:s=${TEXT_CARD_SIZE}x${TEXT_CARD_SIZE}:r=1,format=rgba`,
            "-filter_complex", [
            ...
            "-map", "[out]", "-frames:v", "1", "-update", "1", cardPng,
        ]);

        // composite card onto frame
        const compositedPng = path.join(workDir, "composited.png");
        await run("ffmpeg", [
            "-y", "-i", framePng, "-i", cardPng,
            "-filter_complex", "[0:v][1:v]overlay=x=(main_w-overlay_w)/2:y=(main_h-overlay_h)/2:format=auto[out]",
            "-map", "[out]", "-frames:v", "1", compositedPng,
        ]);
        ...
        if (overlayPath && fs.existsSync(overlayPath)) {
            ...
            const overlayFilter =
                resolvedOverlayBlendMode && resolvedOverlayBlendMode !== "normal"
            ...
            await run("ffmpeg", [
                "-y", "-i", compositedPng, "-i", overlayPath,
                "-filter_complex", overlayFilter,
                "-map", "[out]", "-frames:v", "1", outputPng,
            ]);
        } else {
